...
__WEAK int32_t hal_get_app_version_from_efuse(uint8_t *version)
{
    ...
    if(otp_ef_boot2_anti_rollback_en){
        ...
        bflb_ef_ctrl_read_direct(NULL, ANTI_ROLLBACK_VERSION_OFFSET, value, sizeof(value)/4, 1);
    }
...
__WEAK int32_t hal_set_app_version_to_efuse(uint8_t version)
{
    uint32_t version_low_low = 0;
    uint32_t version_low = 0;
    uint32_t version_high = 0;
    ...
    if(hal_get_app_version_from_efuse(&version_old) != 0) {
        return 1;
    }
    ...
    bflb_ef_ctrl_write_direct(NULL, ANTI_ROLLBACK_VERSION_OFFSET, value, sizeof(value)/4, 1);
